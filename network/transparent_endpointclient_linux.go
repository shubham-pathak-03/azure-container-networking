...
func (client *TransparentEndpointClient) AddEndpoints(epInfo *EndpointInfo) error {
	if _, err := net.InterfaceByName(client.hostVethName); err == nil {
		...
		if err = client.ioShim.Netlink.DeleteLink(client.hostVethName); err != nil {
...
func (client *TransparentEndpointClient) AddEndpointRules(epInfo *EndpointInfo) error {
	...
	for _, ipAddr := range epInfo.IPAddresses {
		...
		if err := addRoutes(client.ioShim.Netlink, client.hostVethName, routeInfoList); err != nil {
			return newErrorTransparentEndpointClient(err.Error())
		}
	}
...
func (client *TransparentEndpointClient) DeleteEndpointRules(ep *endpoint) {
	...
	for _, ipAddr := range ep.IPAddresses {
		...
		routeInfo.Dst = ipNet
		routeInfoList = append(routeInfoList, routeInfo)
		err = deleteRoutes(client.ioShim.Netlink, client.hostVethName, routeInfoList)
		if err != nil {
...
func (client *TransparentEndpointClient) ConfigureContainerInterfacesAndRoutes(epInfo *EndpointInfo) error {
	...
	for _, ipAddr := range epInfo.IPAddresses {
		...
		if err := deleteRoutes(client.ioShim.Netlink, client.containerVethName, []RouteInfo{routeInfo}); err != nil {
			return newErrorTransparentEndpointClient(err.Error())
		}
	}
	...
	if err := addRoutes(client.ioShim.Netlink, client.containerVethName, []RouteInfo{routeInfo}); err != nil {
		return newErrorTransparentEndpointClient(err.Error())
	}
	...
	err := client.ioShim.Netlink.AddOrRemoveStaticArp(netlink.ADD, client.containerVethName, virtualGwNet.IP, client.hostVethMac, false)
	if err != nil {
		return newErrorTransparentEndpointClient(err.Error())
	}
	return nil
