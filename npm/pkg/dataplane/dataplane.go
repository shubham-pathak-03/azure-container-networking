...
func (dp *DataPlane) InitializeDataPlane() error {
	...
	dp.CreateIPSet([]*ipsets.IPSetMetadata{kubeAllSet})
...
func (dp *DataPlane) AddToSet(setNames []*ipsets.IPSetMetadata, podMetadata *PodMetadata) error {
	err := dp.ipsetMgr.AddToSet(setNames, podMetadata.PodIP, podMetadata.PodKey)
	if err != nil {
...
func (dp *DataPlane) RemoveFromSet(setNames []*ipsets.IPSetMetadata, podMetadata *PodMetadata) error {
	err := dp.ipsetMgr.RemoveFromSet(setNames, podMetadata.PodIP, podMetadata.PodKey)
	if err != nil {
...
func (dp *DataPlane) createIPSetsAndReferences(sets []*ipsets.TranslatedIPSet, netpolName string, referenceType ipsets.ReferenceType) error {
	...
	for _, set := range sets {
		dp.ipsetMgr.CreateIPSet([]*ipsets.IPSetMetadata{set.Metadata})
	...
	for _, set := range sets {
		...
		if setType == ipsets.CIDRBlocks {
			...
			err := dp.ipsetMgr.AddToList([]*ipsets.IPSetMetadata{set.Metadata}, getMembersOfTranslatedSets(set.Members))
			if err != nil {
